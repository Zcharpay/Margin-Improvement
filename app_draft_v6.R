...
collookback$sum <- rowSums(as.matrix(select(collookback,-(variable))))
...
dashsummary <- merge(select(collookback,variable,sum), collookfwd, by = "variable", all = TRUE, sort = FALSE)
...
server <- function(input, output){
  ...
  output$GMdeltatable <- renderTable(dashgmpermonth_acty)
